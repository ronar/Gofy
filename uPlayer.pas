...
begin
  ...
  if (num_of_evaded_mobs <> from_lok.lok_mon_count) and (from_lok.lok_id <> 0) then
    ShowMessage('Нельзя сменить локацию! Монстюы есть от котоюых нужно уйти или победить.')
  else
  begin
    ...
    begin
      ...
      fMoves := 0;
      MainForm.lstLog.Items.Add('Игрока засосало в воротики!');
    end
    else
    begin
      ...
      fMoves := fMoves - 1;
      MainForm.lstLog.Items.Add('Игрок перешел в локацию' + GetLokNameByID(fLocation));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    fMoves := fMoves - 1;
    MainForm.lstLog.Items.Add('Игрок перешел на улицу ' + GetLokNameByID(fLocation));
  end;
