...
type
  ...
  private
    fCards: array [1..NUMBER_OF_OW_ENCOUNTER_CARDS] of TLocationCard; // Данные о каждой карте
    function GetCard(n: integer): TLocationCard; // Получает карту из колоды (для трех локаций улицы). Для св-ва.
    //function GetLokation: integer;
    //function GetNom: integer;
    //procedure SetLok;
  public
    ...
    function FindCards(file_path: string): integer; override;
    function DrawCard(n: integer): TLocationCard; // Returns ID of the card on top of the deck
    property cards[n: integer]: TLocationCard read GetCard;
    //property Nom: integer;
    procedure Shuffle();
    //function Get_Card_By_ID(id: integer): TCard; // Нахождение карты по ее ID
  end;
...
// id - number of card in deck
function TOtherWorldCardsDeck.GetCard(n: integer): TLocationCard;
var
  i: integer;
begin
  Result := fCards[n];
...
// n - номер карты
function TOtherWorldCardsDeck.DrawCard(n: integer): TLocationCard;
begin
  DrawCard := fCards[n];//n];
  //Shuffle;
end;
...
begin
  ...
  begin
    ...
    i := i + 1;
    fCards[crd_num].id := StrToInt(Copy(SR.Name, 1, 2));
