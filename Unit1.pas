...
function Find_Cards(file_path: string; var cards: TCardDeck): integer;
var
  F: TextFile;
...
begin
      ...
      begin
        ...
        readln(F, s);
        CloseFile(F);
        TCardDeck(cards).Cards[i].Data := s;
        //Cards^.Cards.Type := CT_UNIQUE_ITEM;
        TCardDeck(cards).Cards[i].ID := StrToInt(Copy(SR.Name, 1, 4));
...
begin
  case Card_Type of
    CT_COMMON_ITEM: begin
      ...
      Common_Items_Count :=  Find_Cards('..\\Gofy\\CardsData\\CommonItems\\', Common_Items_Deck);
      Form1.ComboBox2.Clear;
      Form1.ComboBox2.Text := 'Choose a card';
      for i := 1 to Common_Items_Count do
        Form1.ComboBox1.Items.Add(IntToStr(Common_Items_Deck.Get_Card_ID(i)));
    end; // CT_COMMON_ITEM

    CT_UNIQUE_ITEM: begin
      // задание условий поиска и начало поиска
      Unique_Items_Count := Find_Cards('..\\Gofy\\CardsData\\UniqueItems\\', Unique_Items_Deck);
    ...
    CT_ENCOUNTER: begin
      // Задание условий поиска и начало поиска
      Locations_Count := Find_Cards('..\\Gofy\\CardsData\\Locations\\', Locations_Deck);
      ...
      begin
        Form1.cbLocation.Items.Add(IntToStr(Locations_Deck.Get_Card_ID(i)));
...
begin
  ...
  for i := 1 to 21 do
  begin
    temp := Common_Items_Deck.Cards[i];
    r := random(21);
    Common_Items_Deck.Cards[i] := Common_Items_Deck.Cards[r+1];
  end;
...
begin
  ...
  case c_Cond1 of
  1: begin // Если True
    gPlayer.Take_Action(c_Action1, c_Action1Value);
  end;
  2: begin // Проверка скила
    //ShowMessage('Проверка');
    skill_test := gPlayer.RollADice(gPlayer, c_Choise);
