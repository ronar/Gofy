...
var
  ...
  function GetFirstPlayer: integer; // Получение номера игрока с жетоном првого игрока
 ...
 case gCurrentPhase of
    ...
    PH_MOVE: begin
      ...
      begin
        ...
        MessageDlg('Игрок нашел улики! Аааа-а-аа-аа-ааа супе круто. *Игрок бегает по кругу в порывах радости.*', mtInformation, [mbOK], 0);
      end;
    end;
    PH_ENCOUNTER: begin
    ...
    PH_MYTHOS: begin
      ...
      begin
        if Arkham_Streets[ton(gCurrentMythosCard.fGateSpawn)].SpawnGate(gCurrentMythosCard.fGateSpawn, gates[random(8)+1]) then
          MainForm.lstLog.Items.Add('Появились ворота: ' + GetLokNameByID(gCurrentMythosCard.fGateSpawn));
      end;
      ...
      if drawn_monster <> nil then
        if Arkham_Streets[ton(gCurrentMythosCard.fGateSpawn)].AddMonster(gCurrentMythosCard.fGateSpawn, drawn_monster) then
          MainForm.lstLog.Items.Add('Появился монстр: ' + GetMonsterNameByID(drawn_monster.id));

      // Spawn clue
      if Arkham_Streets[ton(gCurrentMythosCard.fClueSpawn)].AddClue(gCurrentMythosCard.fClueSpawn, 1) then
        MainForm.lstLog.Items.Add('Улика появилась: ' + GetLokNameByID(gCurrentMythosCard.fClueSpawn));
//      btnContinueClick(Sender);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      MainForm.lstLog.Items.Add('Игрок задержан. Пропуск хода.');
      players[fp].bFirstPlayer := False;
      players[fp + 1].bFirstPlayer := true;
      btnContinueClick(Sender);
      exit;
    ...
    players[fp].bFirstPlayer := False;
    players[fp + 1].bFirstPlayer := true;
    MainForm.lstLog.Items.Add('След. игрок.');
    //current_player := fp + 1;
  end
  else
  begin
    ...
    for i := 2 to player_count do
      players[i].bFirstPlayer := false;
    MainForm.lstLog.Items.Add('Все игроки походили. След. фаза.');
    gCurrentPhase := gCurrentPhase + 1;
    if gCurrentPhase > 5 then
      gCurrentPhase := 1;
  ...
  for m := 0 to uMainForm.Monsters_Count-1 do
    ...
    begin
      ...
      begin
        ...
        MainForm.lstLog.Items.Add('Монстр ' + Monsters[m].Name + ' перешел в локацию ' + GetLokNameByID(lok_id));
      end;
    end;
end;
